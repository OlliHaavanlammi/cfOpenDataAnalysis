...
def main():
    ...
    for ii in range(len(division)):
        ...
        columns = ['Userid', 'Name', 'Height', 'Weight', 'Age', 'Regionid',
                   'Regionname', 'Affiliateid', 'Affiliatename', 'Overallrank', 'Overallscore', 
                   str(year)[2:]+'.1_rank', str(year)[2:]+'.1_score',
                   str(year)[2:]+'.2_rank', str(year)[2:]+'.2_score',
                   str(year)[2:]+'.2A_rank', str(year)[2:]+'.2A_score',
        ...
        for i in range(npages):
            ...
            if response != 'No response':
                ...
                for j in range(nathletes):
                    ...
                    ri = int(athlete['entrant']['regionId'])
                    ...
                    w6s = workout_score(athlete['scores'][5]['scoreDisplay'], scaled)
                    w6r = workout_rank(w6s, athlete['scores'][5]['rank'])
                    
                    df = pd.DataFrame([[_id, name, height, weight, age, ri, rn, ai, an,
                                        _or, _os, w1r,w1s, w2r, w2s, w3r, w3s, w4r,
                                        w4s, w5r, w5s, w6r, w6s]], columns=columns)
                    data = data.append(df)
