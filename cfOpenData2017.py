...
def main():
    ...
    for ii in range(len(division)):
        ...
        dname = div_to_name(division[ii])+'_'+scaled_to_name(scaled)+'_'+str(year)
        ...
        # Setup DataFrame
        columns = ['Userid', 'Name', 'Height', 'Weight', 'Age', 'Regionid',
                   'Regionname', 'Affiliateid', 'Overallrank', 'Overallscore', 
                   str(year)[2:]+'.1_rank', str(year)[2:]+'.1_score',
                   str(year)[2:]+'.2_rank', str(year)[2:]+'.2_score',
                   str(year)[2:]+'.3_rank', str(year)[2:]+'.3_score',
                   str(year)[2:]+'.4_rank', str(year)[2:]+'.4_score',
                   str(year)[2:]+'.5_rank', str(year)[2:]+'.5_score']
        data = pd.DataFrame(columns=columns)
    
        # Request first page to set things up
        page = 1
        response = get_page(basepath, headers, division[ii], scaled, page)
        npages = response['totalpages'] #get the number of pages
        #npages = 100 # testing
        save = [500,1000,1500,2000,2500,3000,3500,4000,4500]
        # Loop over pages
        for i in range(npages):
            print(i)
            response = get_page(basepath, headers, division[ii], scaled, i+1)
            if response != 'No response':
                ...
                for j in range(nathletes):
                    athlete = athletes[j]
                    #print(athlete)
                    _id = int(athlete['userid'])
                    name = athlete['name']
                    height = athlete_height(athlete['height'])
                    weight = athlete_weight(athlete['weight'])
                    age = int(athlete['age'])
                    ri = int(athlete['regionid'])
                    rn = athlete['region']
                    ai = int(athlete['affiliateid'])
                    _or = int(athlete['overallrank'])
                    _os = int(athlete['overallscore'])
                    
                    # workouts
                    w1s = workout_score(athlete['scores'][0]['scoredisplay'], scaled)
                    w1r = workout_rank(w1s, athlete['scores'][0]['workoutrank'])
                    w2s = workout_score(athlete['scores'][1]['scoredisplay'], scaled)
                    w2r = workout_rank(w2s, athlete['scores'][1]['workoutrank'])
                    w3s = workout_score(athlete['scores'][2]['scoredisplay'], scaled)
                    w3r = workout_rank(w3s, athlete['scores'][2]['workoutrank'])
                    w4s = workout_score(athlete['scores'][3]['scoredisplay'], scaled)
                    w4r = workout_rank(w4s, athlete['scores'][3]['workoutrank'])
                    w5s = workout_score(athlete['scores'][4]['scoredisplay'], scaled)
                    w5r = workout_rank(w5s, athlete['scores'][4]['workoutrank'])
                    
                    df = pd.DataFrame([[_id, name, height, weight, age, ri, rn, ai, _or,
                                       _os, w1r,w1s, w2r, w2s, w3r, w3s, w4r, w4s, w5r,
                                       w5s]], columns=columns)
                    data = data.append(df)
        ...
        #print('')
        #print(data)
        data.to_pickle(ddir+'/'+dname)
        data.to_csv(path_or_buf=ddir+'/'+dname+'.csv')
